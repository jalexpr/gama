package ru.textanalysis.tawt.gama.converters;
...
public class NumeralsConverter {
	...
	private static final String SPACE_CHARACTER_REG = "\\s";
	private static final String NUMBER_REG = "\\d+";
	private static final String TELEPHONE_NUMBER_REG = "(8|\\+7|7)[0-9]{7,10}";
	...
	private static final String[] ONES_ORDINAL = {
		"нулевых", "первых", "вторых",
		"третьих", "четвертых", "пятых",
		"шестых", "седьмых", "восьмых",
		"девятых", "десятых", "одиннадцатых",
		"двенадцатых", "тринадцатых", "четырнадцатых",
		"пятнадцатых", "шестнадцатых", "семнадцатых",
		"восемнадцатых", "девятнадцатых"
	};
	...
	private static final String[][] ONES_ORDINALS = {
		{"нулевой", "нулевого", "нулевому", "нулевого", "нулевым", "нулевом", "нулевая", "нулевой", "нулевой", "нулевую", "нулевой", "нулевой", "нулевое", "нулевого", "нулевому", "нулевое", "нулевым", "нулевом", "нулевые", "нулевых", "нулевым", "нулевых", "нулевыми", "нулевых"},
	...
	private static final String[][] TENS_ORDINALS = {
		{},
		{"десятый", "десятого", "десятому", "десятом", "десятым", "десятая", "десятой", "десятую", "десятое", "десятые", "десятых", "десятым", "десятыми"},
	...
	public String replaceNumber(List<String> wordsList) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			if ((i == 0 || words[i - 1].matches(SPACE_CHARACTER_REG)) && words.length - 1 > i && word.matches(NUMBER_REG) && (POINT_PATTERN.equals(words[i + 1]) || BRACKET_PATTERN.equals(words[i + 1]))) {
				result.append(word);
				sb.append(result);
			} else if (word.matches(TELEPHONE_NUMBER_REG)) { // проверка на номер телефона
				result.append(word);
				sb.append(result);
				...
				i++;
				word += words[i];
				while (words.length > i + STEP && words[i + STEP].matches("(млн|тыс)")) {
					if (words[i + STEP].equals("млн")) {
						word = String.valueOf(Double.parseDouble(word.replace(",", ".")) * MILLION);
					} else if (words[i + STEP].equals("тыс")) {
						word = String.valueOf(Double.parseDouble(word.replace(",", ".")) * THOUSAND);
					}
					i += STEP;
					if (words.length > i + 3 && words[i + 1].equals(".") && !words[i + STEP].equals("\n")) {
						i++;
					}
				}
				...
				if (word.contains(",") && words.length > i + STEP && words[i + STEP].matches("(руб[а-я]*)|(дол[а-я]*)|(евр[а-я]*)")) {
					result.append(convertFractionalNumberToWords(Double.parseDouble(word.replace(",", ".")), forms, words[i + STEP]));
					words[i + 1] = words[i + STEP] = "";
				} else {
				...
				if (words.length > i + 1) {
					...
					while (words.length > i + STEP && words[i + 1].matches("(\\s)") && words[i + STEP].matches("\\d{3}")) {
						i += STEP;
						number.append(words[i]);
					}
					while (words.length > i + STEP && words[i + STEP].matches("(млн|тыс)")) {
						if (words[i + STEP].equals("млн")) {
							number.append("000_000");
						} else if (words[i + STEP].equals("тыс")) {
							number.append("000");
						}
						i += STEP;
				...
				if (words.length > i + STEP && (words[i + STEP].matches("год|году|годам|годы|года|годов") || words[i + STEP].matches("(января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря)"))) {
					forms = getForms(words, i, !words[i + STEP].matches("годам|годах|годов|годы"));
					String[] date = convertOrdinalToWords(word).split("(?<=( ))");
					for (int j = 0; j < date.length - 1; j++) {
						result.append(date[j]);
					}
					if (words[i + STEP].equals("года")) {
						forms[0] = MorfologyParameters.Case.GENITIVE;
					}
					result.append(convertDateToWords(date[date.length - 1], forms, !words[i + STEP].matches("годам|годах|годов|годы")));
				} else {
					if (words.length > i + STEP) {
	...
	private void changeFirstLetter(String string, StringBuilder sb) {
		if ((!sb.isEmpty() && !string.isEmpty() && sb.charAt(sb.length() - 1) == '\n' || (sb.length() > 1 && sb.substring(sb.length() - 1).matches("[!.?]"))) || sb.isEmpty()) {
	...
	private long[] getForms(String[] words, int i, boolean isSingular) {
		...
		while (i < words.length && !words[i].matches("[.,;:!?]") && !words[i].matches("лет|метров|дней|месяцев|рублей|долларов|раз|километров|минут|часов")) {
			...
			for (Form form : jMorfSdk.getOmoForms(words[i])) {
				if (form.getTypeOfSpeech() == MorfologyParameters.TypeOfSpeech.NOUN && (form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Numbers.class) == MorfologyParameters.Numbers.PLURAL ^ isSingular)) {
					forms[1] = form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Gender.class);
					if (forms[0] == 0 && form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Case.IDENTIFIER) != MorfologyParameters.Case.GENITIVE1 && !(form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Case.IDENTIFIER) == MorfologyParameters.Case.NOMINATIVE && words[i].matches("мая|марта|августа"))) {
	...
	private long[] getFormsWithPreposition(String[] words, int i) {
		int j = STEP;
		while (i - j >= 0 && (words[i - j + STEP].matches("\\b\\S*\\d\\S*\\b") || words[i - j + 1].matches("\\b\\S*\\d\\S*\\b") || words[i - j].matches("\\b\\S*\\d\\S*\\b"))) {
			if (i + 1 < words.length && words[i + 1].matches("году|месяце") && words[i - j].matches("[вВ]")) {
	...
	private String convertNumberToWords(int num, long[] forms, String... buildup) {
		...
		if (num / THOUSAND > 0) {
			if (num / THOUSAND != 1) {
				result.append(convertNumberToWords(num / THOUSAND, new long[]{forms[0] != 0 ? forms[0] : MorfologyParameters.Case.ACCUSATIVE, MorfologyParameters.Gender.FEMININ}));
			}
			result.append(" ").append(getFormedThousand(forms, (num / THOUSAND) % 10)).append(" ");
			num = num % THOUSAND;
		}
		...
		result.append(addOnesToNumber(num, forms, buildup));

		return result.toString().trim();
	}

	private String addOnesToNumber(int num, long[] forms, String... buildup) {
	...
	private String convertDateToWords(String date, long[] forms, boolean isSingular) {
		...
		for (String s : jMorfSdk.getDerivativeFormLiterals(date, MorfologyParameters.TypeOfSpeech.ADJECTIVE_FULL)) {
			for (Form form : jMorfSdk.getOmoForms(s)) {
				if (form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Case.IDENTIFIER) == forms[0] && (form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Numbers.class) == MorfologyParameters.Numbers.PLURAL ^ isSingular)) {
	...
	private String convertFractionalWithSlash(String number, long[] forms) {
		...
		if (secondNumber < 20) {
			words[words.length - 1] = ONES_ORDINAL[secondNumber];
		} else if (secondNumber % 10 != 0) {
			words[words.length - 1] = ONES_ORDINAL[secondNumber % 10];
		} else {
			words[words.length - 1] = TENS_ORDINAL[secondNumber / 10];
		}
	...
	private String convertEnumToWords(String enumer) {
		...
		if (number < 20) {
			words[words.length - 1] = ONES_ORDINAL[number];
		} else if (number % 10 != 0) {
			words[words.length - 1] = ONES_ORDINAL[number % 10];
		} else {
			words[words.length - 1] = TENS_ORDINAL[number / 10];
		}
	...
	private String convertOrdinalToWords(String ordinal) {
		...
		String[] forms;
		if (number < 20) {
			forms = ONES_ORDINALS[number];
		} else if (number % 10 != 0) {
			if ((number % HUNDRED < 20) && (number % HUNDRED > 9)) {
				forms = ONES_ORDINALS[number % HUNDRED];
			} else {
				forms = ONES_ORDINALS[number % 10];
			}
		} else {
			forms = TENS_ORDINALS[(number % HUNDRED) / 10];
		}
	...
	// todo добавить склонение (думаю)
	private String convertFractionalNumberToWords(double number, long[] forms, String... currency) {
	...
	private String convertFractionalNumberToWordsWithoutCurrency(String fractionalStr, int integerPart, long[] forms) {
		...
		String fractionalWords = convertNumberToWords(!fractionalStr.isEmpty() ? Integer.parseInt(fractionalStr) : 0, forms);
	...
	private String convertFractionalCurrencyToWords(String fractionalStr, int integerPart, long[] forms, String currency, double number) {
		int fractionalPart = !fractionalStr.isEmpty() ? Integer.parseInt(fractionalStr) : 0;
		...
		if (forms[0] != 0) {
			...
			for (String s : jMorfSdk.getDerivativeFormLiterals(num, param)) {
				for (Form form : jMorfSdk.getOmoForms(s)) {
					if ((form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Case.IDENTIFIER) == forms[0] && form.getMorphCharacteristics() != 514)) {
						if ((form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Gender.class) == 0) || (form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Gender.class) == forms[1]) || (s.equals("ноль"))) {
							return form.getMyString();
						}
					}
				}
			...
			for (String s : jMorfSdk.getDerivativeFormLiterals(num, param)) {
				for (Form form : jMorfSdk.getOmoForms(s)) {
					...
					} else if (form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Case.IDENTIFIER) == MorfologyParameters.Case.ACCUSATIVE || form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Case.IDENTIFIER) == MorfologyParameters.Case.ACCUSATIVE2) {
						if (form.getMorfCharacteristicsByIdentifier(MorfologyParameters.Gender.class) == forms[1]) {
							return form.getMyString();
						}
					}
